...
server = "sxd1182.database.windows.net"
database = "sxd1182"
username = "CloudSA96001a75"
password = "S150600h*"
...
def createTable():
    ...
    sxdcursor.execute(
        """IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'dbo.earthquakes') AND type in (N'U'))
    ...
    sxdcursor.execute(
        """IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name='idx_location' AND object_id = OBJECT_ID(N'dbo.earthquakes'))
    ...
    sqlite_sxdcursor = sqlite_conn.cursor()

    sqlite_sxdcursor.execute(
    ...
    sqlite_sxdcursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_location ON earthquakes (place)"
    )
    ...
    elapsed_time = close_time - begin_time
...
def upload_csv():
    ...
    with open(file_path, "r", encoding="utf-8") as csvfile:
        ...
        table_exists = sxdcursor.tables(table="earthquakes").fetchone()
        if table_exists:
        ...
        sxdcursor.executemany(insert_query, rows)
        conn.commit()
    ...
    with open(file_path, "r", encoding="utf-8") as csvfile:
        ...
        sqlite_sxdcursor.executemany(insert_query, rows)
        sqlite_conn.commit()
        sqlite_conn.close()
...
@app.route("/randomQueries", methods=["POST"])
def random_queries():
    ...
    for _ in range(num_queries):
        ...
        sxdcursor.execute("SELECT TOP 1 * FROM dbo.earthquakes ORDER BY NEWID()")
        result = sxdcursor.fetchone()
    ...
    return render_template(
        "random_queries.html", results=results, elapsed_time=elapsed_time
    )


@app.route("/restrictedQueries", methods=["POST"])
def restricted_queries():
    ...
    if results is None:
        ...
        query = """SELECT * FROM dbo.earthquakes 
                   WHERE mag > ?"""
        sxdcursor.execute(query, (magnitude,))
        results = sxdcursor.fetchall()
        ...
        print(results)
        return render_template(
            "restricted_queries.html", results=results, elapsed_time=elapsed_time
        )
    else:
        # Results found in cache, return them directly
        return render_template(
            "restricted_queries.html", results=results, elapsed_time=0
        )
