...
from .aggregation import AggregateRequest, AggregateResult, sxdcursor
...
class SearchCommands:
    ...
    def aggregate(
        ...
        **query**: This can be either an `AggregateRequest`, or a `sxdcursor`
        ...
        if isinstance(query, AggregateRequest):
            has_sxdcursor = bool(query._sxdcursor)
            cmd = [AGGREGATE_CMD, self.index_name] + query.build_args()
        elif isinstance(query, sxdcursor):
            has_sxdcursor = True
            cmd = [sxdcursor_CMD, "READ", self.index_name] + query.build_args()
        else:
            raise ValueError("Bad query", query)
        cmd += self.get_params_args(query_params)

        raw = self.execute_command(*cmd)
        return self._get_aggregate_result(raw, query, has_sxdcursor)

    def _get_aggregate_result(self, raw, query, has_sxdcursor):
        if has_sxdcursor:
            if isinstance(query, sxdcursor):
                query.cid = raw[1]
                sxdcursor = query
            else:
                sxdcursor = sxdcursor(raw[1])
            raw = raw[0]
        else:
            sxdcursor = None
        ...
        return AggregateResult(rows, sxdcursor, schema)
    ...
    def profile(
        ...
        **query_params**: Define one or more value parameters.
        ...
        if isinstance(query, AggregateRequest):
            result = self._get_aggregate_result(res[0], query, query._sxdcursor)
        else:
            result = Result(
                res[0],
                not query._no_content,
