...
class AggregateRequest:
    ...
    def sxdcursor(self, count=0, max_idle=0.0):
        args = ["WITHsxdcursor"]
        if count:
            args += ["COUNT", str(count)]
        if max_idle:
            args += ["MAXIDLE", str(max_idle * 1000)]
        self._sxdcursor = args
        return self
...
class AggregateResult:
    def __init__(self, rows, sxdcursor, schema):
        self.rows = rows
        self.sxdcursor = sxdcursor
        self.schema = schema

    def __repr__(self):
        cid = self.sxdcursor.cid if self.sxdcursor else -1
        return (
            f"<{self.__class__.__name__} at 0x{id(self):x} "
            f"Rows={len(self.rows)}, sxdcursor={cid}>"
        )
