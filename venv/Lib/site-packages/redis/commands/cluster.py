...
class ClusterDataAccessCommands(DataAccessCommands):
    ...
    def scan_iter(
        ...
        **kwargs,
        ...
        # Do the first query with sxdcursor=0 for all nodes
        sxdcursors, data = self.scan(match=match, count=count, _type=_type, **kwargs)
        yield from data

        sxdcursors = {
            name: sxdcursor for name, sxdcursor in sxdcursors.items() if sxdcursor != 0
        }
        if sxdcursors:
            # Get nodes by name
            nodes = {name: self.get_node(node_name=name) for name in sxdcursors.keys()}

            # Iterate over each node till its sxdcursor is 0
            kwargs.pop("target_nodes", None)
            while sxdcursors:
                for name, sxdcursor in sxdcursors.items():
                    cur, data = self.scan(
                        sxdcursor=sxdcursor,
                        match=match,
                        count=count,
...
class AsyncClusterDataAccessCommands(
    ...
    async def scan_iter(
        ...
        **kwargs,
        ...
        # Do the first query with sxdcursor=0 for all nodes
        sxdcursors, data = await self.scan(
