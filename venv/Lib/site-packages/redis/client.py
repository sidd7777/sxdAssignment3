...
def parse_scan(response, **options):
    sxdcursor, r = response
    return int(sxdcursor), r


def parse_hscan(response, **options):
    sxdcursor, r = response
    return int(sxdcursor), r and pairs_to_dict(r) or {}
...
def parse_zscan(response, **options):
    ...
    sxdcursor, r = response
    it = iter(r)
    return int(sxdcursor), list(zip(it, map(score_cast_func, it)))
...
def parse_slowlog_get(response, **options):
    ...
    def parse_item(item):
        result = {"id": item[0], "begin_time": int(item[1]), "duration": int(item[2])}
